...
void MyTrackballSetup() {

    GPIO_InitTypeDef GPIO_InitStructure;

    RCC_APB2PeriphClockCmd(BALL_MT_PERIPH, ENABLE);
    GPIO_InitStructure.GPIO_Pin = BALL_MT_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(BALL_MT_PIN_PORT, &GPIO_InitStructure);

    RCC_APB2PeriphClockCmd(BALL_SS_PERIPH, ENABLE);
    GPIO_InitStructure.GPIO_Pin = BALL_SS_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(BALL_SS_PIN_PORT, &GPIO_InitStructure);
...
void MyTrackballInit() {
	...
	volatile uint8_t product_id = adns_read_reg(0x00);
	volatile uint8_t inverse_product_id = adns_read_reg(0x3F);
//	volatile uint8_t srom_version = adns_read_reg(0x2A);
//	volatile uint8_t motion = adns_read_reg(0x02);
...
void adns_com_begin() {
	GPIO_ResetBits(BALL_SS_PIN_PORT,  BALL_SS_PIN);
}

void adns_com_end() {
	GPIO_SetBits(BALL_SS_PIN_PORT,  BALL_SS_PIN);
}
...
byte adns_read_reg(byte reg_addr){
  ...
  SPI_I2S_SendData(SPI2, reg_addr & 0x7f);
  ...
  while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_RXNE) == RESET);
  byte data = SPI_I2S_ReceiveData(SPI2);
...
void adns_upload_firmware() {
  ...
  while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE) == RESET);
  SPI_I2S_SendData(SPI2, SROM_Load_Burst | 0x80); // write burst destination adress
  while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_RXNE) == RESET);
  SPI_I2S_ReceiveData(SPI2);
  ...
  for(int i = 0; i < firmware_length; i++) {
	  while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE) == RESET);
	  SPI_I2S_SendData(SPI2, firmware_data[i]);
	  while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_RXNE) == RESET);
	  SPI_I2S_ReceiveData(SPI2);
    //delayMicroseconds(15);
	DelayUs(15);
